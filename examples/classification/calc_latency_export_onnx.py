...
def export_all(exp_nr, calc_latency=False, ext_name='cpu', device_id=0,
    ...
    if exp_nr == 1:
        ...
        if calc_latency:
            estim_real, estim_accum = init_calc_latency(
                output,
                ext_name=ext_name,
                device_id=device_id
            )
        ...
        zn.save_net_nnp(OUTPUT_DIR + 'zn', input, output,
                        ...
                        func_real_latency=estim_real,
                        func_accum_latency=estim_accum
                        )

        # SAVE ALL MODULES
        zn.save_modules_nnp(OUTPUT_DIR + 'zn', active_only=True,
                            calc_latency=calc_latency,
                            func_latency=estim_accum
                            )
    ...
    if exp_nr == 2:
        ...
        for i in range(0, N):
            ...
            if calc_latency:
                estim_real, estim_accum = init_calc_latency(
                    output, ext_name=ext_name, device_id=device_id)
    ...
    if exp_nr == 20:
        ...
        # Measure latency on same zoph network N times using
        # LatencyEstimator or LatencyGraphEstimator
        N = 10
        for i in range(0, N):
            if calc_latency:
                estim_real, estim_accum = init_calc_latency(
    ...
    if exp_nr == 5:
        ...
        for i in range(0, N):
            ...
            # it seems the last affine layer
            # cannot be converted to ONNX (!?),
            # here is the export without it
            # mobile_net.save_net_nnp(OUTPUT_DIR + 'mn' + str(i),
            #                         input, output.parent.inputs[0],
            #                         save_latency=True
            #                         )

            if calc_latency:
                mobile_net.calc_latency_modules(
                    OUTPUT_DIR + 'mn' + str(i), output,
                    func_latency=estim_accum)
    ...
    if exp_nr == 50:
        ...
        for i in range(0, N):
            print('****************** RUN ********************')
            mobile_net.save_net_nnp(OUTPUT_DIR + 'mn' + str(i),
                                    input, output, save_latency=True)

        # mobile_net.save_modules_nnp()
        # it seems the last affine layer
        # cannot be converted to ONNX (!?),
        # here export without it
        # mobile_net.save_net_nnp(OUTPUT_DIR + 'mn' + str(i),
        #                         input, output.parent.inputs[0],
        #                         save_latency=True
        #                         )
    ...
    if exp_nr == 6:
        ...
        for network in existing_networks:
            ...
            for block_lat in all_blocks:
                ...
                # Layer-type-wise latencies tested for Zoph
                # and for Random Wired networks
        ...
        print('NETWORK REAL, ACCUM by graph analysis, ',
              'ACCUM by module analysis, of CONVs,  of BNs,  of RELUs,',
    ...
    if exp_nr == 7:
        ...
        for filename in glob.glob('./logs/zoph/one_net/zn/**/*.nnp',
                                  recursive=True):
