...
class OFASearcher(Searcher):
	...
	def train_on_batch(self, epoch, n_iter, key='train'):
		...
		for _, data in enumerate(batch):
			...
			#p['loss'].forward(clear_no_need_grad=True, function_post_hook=lambda f: print(f.name))
			...
			#p['loss'].backward(clear_buffer=True, function_post_hook=lambda f: print(f.name))
			loss = p['loss'].d.copy() #slow? --> may be replaced by NdArray
