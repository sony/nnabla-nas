...
class DynamicSE(SEModule):
	...
	def get_active_reduce_weight(self, num_mid, in_channel, group=None):
		if group is None or group == 1:
			...
			#sub_in_channels = in_channel // group
			#sub_filters = torch.chunk(self.fc.reduce._W[:num_mid, :, :, :], group, dim=1)
			#return torch.cat([
	...
	def call(self, input, group=None):
		...
		#y =  F.mean(F.mean(input, axis=3, keepdims=True), axis=2, keepdims=True)
...
class DynamicConv2d(Mo.Module):
	def __init__(self, max_in_channels, max_out_channels, 
		...
		self.conv = Mo.Conv(
			...
			#pad=(0, 0), stride=self._stride, dilation=(1, 1), with_bias=False
		)
	...
	def call(self, input, out_channel=None):
		...
		in_channel = input.shape[1]
		#filters = self.get_activate_filter(out_channel, in_channel).contiguous()
...
class DynamicBatchNorm2d(Mo.Module):
	...
	def bn_forward(x, bn: Mo.BatchNormalization, feature_dim, training):
		#if bn._n_features == feature_dim or DynamicBatchNorm2d.SET_RUNNING_STATISTICS:
		if DynamicBatchNorm2d.GET_STATIC_BN or DynamicBatchNorm2d.SET_RUNNING_STATISTICS:
			...
			#bn._mean.data[:, :feature_dim, :, :] = smean.data
			#bn._var.data[:, :feature_dim, :, :] = svar.data
			if training:
	...
	def call(self, input):
		...
		"""
		if self.training:
			sbeta, sgamma = self.bn._beta[:, :feature_dim, :, :], self.bn._gamma[:, :feature_dim, :, :]
			smean = nn.Variable(sbeta.shape)
			svar = nn.Variable(sbeta.shape)
			smean.data.copy_from(self.bn._mean.data[:, :feature_dim, :, :])
			svar.data.copy_from(self.bn._var.data[:, :feature_dim, :, :])
			bn = F.batch_normalization(
				input, sbeta, sgamma, smean, svar, batch_stat=self.training)
			self.bn._mean.data[:, :feature_dim, :, :] = smean.data
			self.bn._var.data[:, :feature_dim, :, :] = svar.data
			return bn
		else:
			mean = F.mean(input, axis=(0, 2, 3), keepdims=True)
			mean_broadcast = F.broadcast(mean, input.shape)
			var = F.mean(F.squared_error(input, mean_broadcast), axis=(0, 2, 3), keepdims=True)
			return F.batch_normalization(input, 
										self.bn._beta[:, :feature_dim, :, :], 
										self.bn._gamma[:, :feature_dim, :, :],
										mean, 
										var, 
...
class DynamicSeparableConv2d(Mo.Module):
	...
	def __init__(self, max_in_channels, kernel_size_list, stride=(1, 1), dilation=(1, 1)):
		...
		#	self._max_in_channels, self._max_in_channels, 
		#	kernel=(max(self._kernel_size_list), max(self._kernel_size_list)), 
		#	stride=self._stride, dilation=self._dilation, group=self._max_in_channels, with_bias=False)
		...
		if self.KERNEL_TRANSFORM_MODE is not None:
			...
			for i in range(len(self._ks_set) - 1):
				...
				#scale_param = nn.Variable.from_numpy_array(np.eye(ks_small ** 2), need_grad=True)
	...
	def call(self, input, kernel=None):
		...
		#filters = self.get_active_filter(in_channel, kernel).contiguous()
...
class DynamicLinear(Mo.Module):
	...
	def call(self, input, out_features=None):
		...
		weight = self.get_active_weight(in_features, out_features) # not sure
