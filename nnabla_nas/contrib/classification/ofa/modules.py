...
class My2DLayer(Mo.Module):
	...
	def __init__(self, in_channels, out_channels,
		...
		if self._use_se:
			#self.add_module('se', SEModule(self._out_channels))
...
class ConvLayer(My2DLayer):
	...
	def weight_op(self):
		...
		if self._has_shuffle and self._group > 1:
			...
			#weight_dict['shuffle'] = Mo.ShuffleLayer(self._group)
...
class MBConvLayer(Mo.Module):
	def __init__(self, in_channels, out_channels, 
		...
		depth_conv_modules =[
			...
			#('dwconv', Mo.DwConv(feature_dim, kernel, pad=pad, stride=stride, with_bias=False)),
			('bn', Mo.BatchNormalization(feature_dim, 4)),
			('act', build_activation(self._act_func, inplace=True)),
		]
		if self._use_se:
